...
while(1):
	...
	#image = cv2.imread("examples"+".png")
	
	crop_img = image[465:490, 1400:1450]
	gray = cv2.cvtColor(crop_img, cv2.COLOR_BGR2GRAY)
	retOne,thresh1 = cv2.threshold(gray,80,255,cv2.THRESH_BINARY)
	resized = cv2.resize(thresh1, (100,100), interpolation = cv2.INTER_LINEAR             )


	cropt_img_partTwo = image[442:469, 1461:1669]
	#gray_two = cv2.cvtColor(cropt_img_partTwo, cv2.COLOR_BGR2GRAY)
	retTwo,thresh3 = cv2.threshold(cropt_img_partTwo,66,255,cv2.THRESH_BINARY)
	resized_TWO = cv2.resize(thresh3,  (500,45), interpolation = cv2.INTER_NEAREST             )
		
	crop_img_two = image[442:469, 251:459]
	gray_three = cv2.cvtColor(crop_img_two, cv2.COLOR_BGR2GRAY)
	retThree,thresh2 = cv2.threshold(gray_three,66,255,cv2.THRESH_BINARY)
	resized_THREE = cv2.resize(thresh2,  (500,45), interpolation = cv2.INTER_NEAREST             )

	crop_img_two_parttwo = image[465:490, 470:520]
	#gray_two_parttwo = cv2.cvtColor(crop_img_two_parttwo, cv2.COLOR_BGR2GRAY)
	retFour,thresh4 = cv2.threshold(crop_img_two_parttwo,80,255,cv2.THRESH_BINARY)
	resized_two = cv2.resize(thresh4, (100,100), interpolation = cv2.INTER_LINEAR              )

	
	#cv2.imshow("nesks",resized)
	#cv2.imshow("crop_two",resized)
	#cv2.imshow("crop_four",resized_two)
	...
	Left_Num= pytesseract.image_to_string(resized_two,lang="eurostile", config='digits')
